  ...
  if (success) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-4 gap-4 text-center">
        ...
        <h2 className="text-xl font-bold text-[#f0f0f0]">Check your email</h2>
        <p className="text-sm text-[#8a8a8a]">
          If an account exists for <strong>{email}</strong>, we&apos;ve sent a
          password reset link.
        </p>
        <Link href="/login" className="auth-link text-sm font-medium">
          Back to Login
        </Link>
      </div>
    );
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-4">
      ...
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
        ...
        <Link href="/login" className="auth-link text-center text-sm">
          Back to Login
        </Link>
      </form>
    </div>
