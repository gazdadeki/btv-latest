  ...
  return (
    <div className="flex items-center gap-2">
      {met ? (
        ...
        <div className="w-4 h-4 rounded-full border-2 border-[#3a3530] shrink-0" />
      )}
      <span
        className={cn("text-xs", met ? "text-[#2a9d8f]" : "text-[#8a8a8a]")}
      >
        {label}
      </span>
    </div>
...
function ResetPasswordForm() {
  ...
  if (!token) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-4 gap-4 text-center">
        ...
        <h2 className="text-xl font-bold text-[#f0f0f0]">Invalid Reset Link</h2>
        <p className="text-sm text-[#8a8a8a]">
          This link is invalid or has expired. Please request a new reset.
        </p>
  ...
  if (success) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-4 gap-4 text-center">
        <CheckCircle className="w-16 h-16 text-[#2a9d8f]" />
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setErrors({
        submit: err instanceof Error ? err.message : "Reset failed",
      });
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-4">
      <div className="h-8" />
      <Lock className="w-16 h-16 text-[#2a9d8f] mx-auto" />
      <div className="h-6" />
      ...
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
        <div>
          <div className="relative">
            ...
            <input
              ...
              className="auth-input pl-10 pr-10 py-2.5 text-sm"
            />
            <button
              type="button"
        ...
        <div className="auth-requirements p-3 flex flex-col gap-1.5">
