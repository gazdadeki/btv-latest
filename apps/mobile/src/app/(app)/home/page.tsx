...
import { Filter, Calendar, Play, CheckCircle, XCircle, History, ChevronDown, ChevronUp } from 'lucide-react';
...
import type { Game, Reservation, GameStatusFilter, ScheduleSection } from '@/types';
import { GAME_STATUS_DISPLAY, reservationIsActive, reservationIsCompleted, reservationIsConfirmed, availableSlotsCount, gameIsCancelled, gameIsFinished, gameIsInProgress } from '@/types';
...
function GameCard({ game, hasActiveReservation, onTap }: { game: Game; hasActiveReservation: boolean; onTap: () => void }) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <span className="text-lg font-bold text-gray-900">{formatTime(game.scheduledStartTime)}</span>
        <span className={cn('flex items-center gap-1 px-2 py-0.5 rounded text-xs font-bold border', colorClass)}>
          <Icon className="w-3 h-3" />
          {cfg.label}
        </span>
      </div>
      <div className="flex items-center gap-1 text-sm text-gray-500">
        <span>{availableSlotsCount(game)} / {game.slots.length} slots available</span>
      </div>
      {game.isExclusiveToGold && (
        <div className="mt-2 flex items-center gap-1">
          <span className="text-xs font-bold text-amber-600">★ Gold Exclusive</span>
        </div>
      )}
      {hasActiveReservation && (
        <div className="mt-2 bg-blue-50 border border-blue-200 rounded p-2 flex items-center gap-2">
          <span className="text-xs text-blue-700">You have an active reservation</span>
        </div>
      )}
    </button>
  );
...
function GamesHistorySection({ reservations, onGameTap }: { reservations: Reservation[]; onGameTap: (id: number) => void }) {
    ...
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm mb-3">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-500">({uniqueGameIds.length} past game{uniqueGameIds.length !== 1 ? 's' : ''})</span>
        </div>
        ...
        <div className="border-t border-gray-100 divide-y divide-gray-100">
          {uniqueGameIds.map(gameId => {
            ...
            return (
              ...
              >
                <div>
                  <p className="text-sm font-medium text-gray-800">Game #{gameId}</p>
                  <p className="text-xs text-gray-500">Reserved: {formatDate(r.createdAt)}</p>
                </div>
...
export default function HomePage() {
  ...
  const { data: scheduleSections = [], isLoading, error, refetch } = useQuery({
  ...
  const activeReservations = myReservations.filter(r => reservationIsActive(r));
  const hasActiveReservation = activeReservations.length > 0;
  const finishedReservations = myReservations.filter(r => reservationIsCompleted(r) || reservationIsConfirmed(r));
  ...
  const filterChips = [
    ...
    { key: 'includeInProgress' as const,  label: 'In Progress', color: 'orange' },
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
        ...
        <button onClick={() => setShowFilterSheet(true)} className="p-2 text-gray-500 hover:text-gray-700">
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isActive ? colorStyles : 'border-gray-200 text-gray-400 bg-transparent',
              )}
            >
              {label}
            </button>
      ...
      >
        <button onClick={handleRefresh} className="w-full text-xs text-gray-400 py-1 mb-1 text-center">Pull to refresh</button>
        ...
        {scheduleSections.map((section: ScheduleSection) => (
          <div key={section.id} className="mb-4">
            <h2 className="text-lg font-bold text-gray-800 px-2 py-2">{section.name}</h2>
            {(section.games ?? []).map((game: Game) => {
              const ownReservation = activeReservations.find(r => r.gameId === game.id);
              return (
                <GameCard
                  key={game.id}
                  game={game}
                  hasActiveReservation={!!ownReservation && ownReservation.gameId === game.id}
      ...
      {showFilterSheet && (
        <div className="fixed inset-0 z-50 flex items-end" onClick={() => setShowFilterSheet(false)}>
          ...
          <div className="relative bg-white w-full max-w-lg mx-auto rounded-t-2xl p-4 pb-8" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900">Filter Games</h2>
              <button onClick={() => setFilter(DEFAULT_FILTER)} className="text-sm text-indigo-600">Reset</button>
            </div>
            ...
            <Button size="full" onClick={() => setShowFilterSheet(false)}>Apply Filters</Button>
          </div>
        </div>
      )}
    </div>
