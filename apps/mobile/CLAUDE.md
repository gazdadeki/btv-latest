...
- **Manifest** (`public/manifest.json`): `display: standalone`, dark `theme_color`/`background_color` (`#0f0e0c`), `id: "/"`, `start_url: /home`, `scope: /`, plus `categories` and `shortcuts` (Arena/Store). Icons are split by purpose — `any` (192, 512) **and** a dedicated `maskable` (512) with safe-zone padding. Never put `any maskable` on one icon (Android crops it).
- **Icons** are generated by `scripts/generate-icons.mjs` (`npm run icons`, uses `sharp`) — a deterministic gold-on-dark placeholder. Outputs `icons/icon-{192,512}.png`, `icons/icon-maskable-512.png`, root `apple-touch-icon.png` (180, **opaque** — iOS ignores alpha/maskable) and `favicon-32x32.png`. Swap in real art (and add real manifest `screenshots`) before launch, then re-run.
- **iOS metadata** (`src/app/layout.tsx`): `appleWebApp.capable`, `statusBarStyle: "black"` (opaque dark bar — do **not** use `black-translucent`, no layout pads `safe-area-inset-top`), `viewportFit: cover`, `formatDetection` off, apple-touch-icon via `metadata.icons.apple`.
- **Install UX** (`components/install-prompt.tsx`): Android/Chromium → native "Install app" button; iOS Safari → manual Add-to-Home-Screen steps; other iOS browsers/in-app webviews → "open in Safari" hint; renders nothing in standalone. The `beforeinstallprompt` event is captured **app-wide and early** by `lib/pwa-install.ts` (module-level listener), loaded on every page via `<PwaInstallListener />` in the root layout — a route-local `useEffect` listener misses the event when navigating in from another route. Platform detection: `lib/platform.ts`.
- **`/install`** (`src/app/install/`): public, unauthenticated landing/install page; a standalone route **outside** `(app)`/`(auth)` (no AuthProvider/WS/Stripe). Shareable "download link" = `https://app.<domain>/install`. No browser-fallback link — players are expected to install.
- **Offline fallback**: `next.config.ts` `fallbacks: { document: '/offline' }` → `src/app/offline/page.tsx` (self-contained, no network/auth).
- **Middleware**: `/install` and `/offline` are fully ungated (early `return` before the auth checks) so they render for every auth state; `apple-touch-icon` is in the matcher exclusions.
- **Build artifacts** (`sw.js`, `swe-worker-*.js`, `workbox-*.js`) are gitignored and regenerated on build — never commit them. `firebase-messaging-sw.js` is hand-written source and **is** tracked.
...
- Toasts: `closeButton`, 3s auto-dismiss. Swipe-to-dismiss is sonner v2's default behavior — there is no `swipeToDismiss` prop (it was removed; passing it is a type error).
