...
export class Init1765735271455 implements MigrationInterface {
  name = 'Init1765735271455';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`schedules\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`description\` text NULL, \`createdBy\` int NOT NULL, \`recurrenceType\` enum ('WEEKLY', 'MONTHLY', 'YEARLY', 'ONCE') NOT NULL, \`recurrenceDays\` json NULL, \`recurrencePattern\` json NULL, \`slotsPerEvent\` int NOT NULL, \`reservationCost\` decimal(10,2) NOT NULL, \`instantReservationCost\` decimal(10,2) NULL, \`confirmationWindowMinutes\` int NOT NULL, \`refundPolicy\` enum ('FULL', 'PARTIAL', 'NONE') NOT NULL DEFAULT 'NONE', \`refundPercentage\` int NULL, \`isExclusiveToGold\` tinyint NOT NULL DEFAULT 0, \`firstEventStartTime\` time NOT NULL, \`autoStartNextAfterMinutes\` int NULL, \`eventsPerDay\` int NOT NULL DEFAULT '1', \`spacingAfterFinishMinutes\` int NULL, \`teamAName\` varchar(255) NOT NULL, \`teamBName\` varchar(255) NOT NULL, \`preAssignedUsers\` json NULL, \`reminderMinutesBefore\` json NULL, \`url\` varchar(500) NULL DEFAULT 'https://youtube.com', \`isActive\` tinyint NOT NULL DEFAULT 1, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createdByUserId\` int NULL, INDEX \`IDX_fc1e3baad88f7acd26b212377b\` (\`isActive\`), INDEX \`IDX_898b8e51fa21a1c38039bb054c\` (\`createdBy\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`events\` (\`id\` int NOT NULL AUTO_INCREMENT, \`scheduleId\` int NOT NULL, \`status\` enum ('CREATED', 'IN_PROGRESS', 'FINISHED', 'CANCELLED') NOT NULL DEFAULT 'CREATED', \`scheduledStartTime\` datetime NOT NULL, \`actualStartTime\` datetime NULL, \`actualEndTime\` datetime NULL, \`durationMinutes\` int NULL, \`teamAName\` varchar(255) NOT NULL, \`teamBName\` varchar(255) NOT NULL, \`isExclusiveToGold\` tinyint NOT NULL DEFAULT 0, \`winningTeam\` enum ('A', 'B') NULL, \`mvpUserId\` int NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`IDX_37f9cc142b9f8cffeaf01cde0f\` (\`scheduledStartTime\`), INDEX \`IDX_03dcebc1ab44daa177ae9479c4\` (\`status\`), INDEX \`IDX_20130ace5ce165fcc5ca20d79e\` (\`scheduleId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`slots\` (\`id\` int NOT NULL AUTO_INCREMENT, \`eventId\` int NOT NULL, \`slotNumber\` int NOT NULL, \`team\` enum ('A', 'B') NOT NULL, \`isReserved\` tinyint NOT NULL DEFAULT 0, \`reservedByUserId\` int NULL, \`isPreAssigned\` tinyint NOT NULL DEFAULT 0, \`preAssignedUserId\` int NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`IDX_dda4708db37cb3e90144e539d7\` (\`team\`), INDEX \`IDX_665c6a54ef9c49866a7bfa21bc\` (\`eventId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`reservations\` (\`id\` int NOT NULL AUTO_INCREMENT, \`slotId\` int NOT NULL, \`userId\` int NOT NULL, \`eventId\` int NOT NULL, \`status\` enum ('RESERVED', 'CONFIRMED', 'CANCELLED', 'EXPIRED') NOT NULL DEFAULT 'RESERVED', \`reservationCostPaid\` decimal(10,2) NOT NULL, \`confirmationCostPaid\` decimal(10,2) NOT NULL DEFAULT '0.00', \`totalCostPaid\` decimal(10,2) NOT NULL, \`discountApplied\` decimal(10,2) NULL, \`originalCost\` decimal(10,2) NULL, \`reservedAt\` datetime NOT NULL, \`confirmedAt\` datetime NULL, \`cancelledAt\` datetime NULL, \`expiredAt\` datetime NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`IDX_c42f5dcdd13d6e63ee44b4cb23\` (\`status\`), INDEX \`IDX_c6fda79964e4d3a3e3f9843fbc\` (\`eventId\`), INDEX \`IDX_aa0e1cc2c4f54da32bf8282154\` (\`userId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`events\` ADD CONSTRAINT \`FK_20130ace5ce165fcc5ca20d79ec\` FOREIGN KEY (\`scheduleId\`) REFERENCES \`schedules\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`events\` DROP FOREIGN KEY \`FK_20130ace5ce165fcc5ca20d79ec\``,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `DROP INDEX \`IDX_20130ace5ce165fcc5ca20d79e\` ON \`events\``,
    );
    await queryRunner.query(
      `DROP INDEX \`IDX_03dcebc1ab44daa177ae9479c4\` ON \`events\``,
    );
    await queryRunner.query(
      `DROP INDEX \`IDX_37f9cc142b9f8cffeaf01cde0f\` ON \`events\``,
    );
