  ...
  const handleSave = async () => {
    if (isEdit) {
      const data: Record<string, unknown> = {
        email: form.email,
        username: (form.username as string) || null,
        role: form.role,
    ...
    >
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <FormRow label={isEdit ? "Email" : "Email *"}>
            <FormInput
              form={form}
              field="email"
              setForm={setForm}
        ...
        <FormRow label={isEdit ? "Password (blank to keep)" : "Password *"}>
          <FormInput
            form={form}
            field="password"
            setForm={setForm}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <FormRow label="Tier">
            <select
              value={String(form.subscriptionTier || "")}
              onChange={(e) =>
