...
import { GAME_STATUS_COLORS, RESERVATION_STATUS_COLORS, teamDisplay } from '@/constants';
...
export function GameDetailDialog({ game, onClose, onMutated }: GameDetailDialogProps) {
  ...
  const mutated = () => { if (game) onMutated(game.id); };
  ...
  const isModifiable = game.status !== 'FINISHED' && game.status !== 'CANCELLED';
  const reservedCount = game.reservations?.filter((r) => r.status === 'RESERVED').length || 0;
  const confirmedCount = game.reservations?.filter((r) => r.status === 'CONFIRMED').length || 0;

  return (
    <>
      <Dialog open onClose={handleClose} title="Game Details" className="max-w-4xl">
        <Tabs
          tabs={[
          ...
          <div>
            <table className="w-full text-sm">
              <tbody className="divide-y divide-gray-100">
                <tr><td className="py-2 font-medium w-1/3">ID</td><td>{game.id}</td></tr>
                <tr><td className="py-2 font-medium">Schedule</td><td>{game.schedule?.name || `#${game.scheduleId}`}</td></tr>
                <tr>
                  <td className="py-2 font-medium">Status</td>
                  <td><StatusBadge status={game.status} colorMap={GAME_STATUS_COLORS} /></td>
                </tr>
                <tr>
                ...
                <tr><td className="py-2 font-medium">Actual Start</td><td>{game.actualStartTime ? formatDate(game.actualStartTime) : 'N/A'}</td></tr>
                <tr><td className="py-2 font-medium">Actual End</td><td>{game.actualEndTime ? formatDate(game.actualEndTime) : 'N/A'}</td></tr>
                <tr><td className="py-2 font-medium">Duration</td><td>{game.durationMinutes ? `${game.durationMinutes} min` : 'N/A'}</td></tr>
                {game.winningTeam && (
                  <tr><td className="py-2 font-medium">Winner</td><td>{game.winningTeam === 'A' ? game.teamAName : game.teamBName}</td></tr>
                )}
                <tr><td className="py-2 font-medium">Created</td><td>{game.createdAt ? formatDate(game.createdAt) : 'N/A'}</td></tr>
                <tr><td className="py-2 font-medium">Updated</td><td>{game.updatedAt ? formatDate(game.updatedAt) : 'N/A'}</td></tr>
              </tbody>
            </table>
          ...
          <div>
            ...
            <div className="overflow-x-auto border border-gray-200 rounded-lg">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {(game.slots || []).sort((a, b) => a.slotNumber - b.slotNumber).map((slot) => (
                    <tr key={slot.id}>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex gap-1">
                          {!slot.isReserved && game.status === 'CREATED' && (
                            <Button variant="success" size="xs" onClick={() => setSlotAssign({ gameId: game.id, slotId: slot.id })} title="Assign User">
                              <i className="fas fa-user-plus" />
                            </Button>
                          )}
                          {slot.isReserved && isModifiable && (
                            <Button variant="danger" size="xs" onClick={() => handleKick(slot.id)} title="Kick User">
          ...
          <div>
            {isModifiable && (
              <div className="mb-3 flex gap-2">
                ...
                {confirmedCount > 0 && (
                  <Button variant="danger" size="sm" onClick={handleCancelAllConfirmations}>
                ...
                <tbody className="divide-y divide-gray-100">
                  {(game.reservations || []).map((r) => (
                    <tr key={r.id}>
                      ...
                      <td className="px-3 py-2">
                        <StatusBadge status={r.status} colorMap={RESERVATION_STATUS_COLORS} />
                      </td>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex gap-1">
                          {r.status === 'RESERVED' && isModifiable && (
                            <Button variant="success" size="xs" onClick={() => handleConfirmReservation(r.slotId)} title="Confirm">
                          ...
                          {(r.status === 'EXPIRED' || r.status === 'CANCELLED') && isModifiable && (
                            <Button
                              ...
                              onClick={() => setReReserve({
                                gameId: game.id, slotId: r.slotId, userId: r.userId,
        ...
        {reReserve && (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <StatusBadge status={reReserve.status} colorMap={RESERVATION_STATUS_COLORS} />
            </div>
