...
export default function SchedulesPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const result = (await api.generateGamesForSchedule(
        generateTarget,
        utcDate,
      )) as { gamesCreated?: number };
      toast.success(
        `Generated ${result.gamesCreated || 0} game(s) for ${generateDate}`,
      );
  ...
  const handleCancelGames = (id: number) => {
    confirm({
      ...
      message:
        "Cancel all CREATED games for this schedule? This cannot be undone.",
      variant: "danger",
      onConfirm: async () => {
        try {
        ...
        return (
          <div className="flex gap-1">
            ...
            <Button
              variant="orange"
              size="xs"
              onClick={() => handleCancelGames(s.id)}
              title="Cancel Games"
            >
      ...
      >
        <div className="space-y-4">
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-sm text-yellow-800">
            This will cancel all dangling games for this schedule and generate
            new games for the selected date.
          </div>
