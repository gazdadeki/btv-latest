...
export function RecurrenceForm({ form, setForm }: RecurrenceFormProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {type === "WEEKLY" && (
        <div>
          ...
          <div className="flex gap-2">
            {WEEKDAYS.map((d, i) => {
              // Backend uses getUTCDay() convention: 0=Sun, 1=Mon, ..., 6=Sat.
              // WEEKDAYS is Mon-first, so map index 6 (Sun) → 0.
              const day = (i + 1) % 7;
                  ...
                  onClick={() =>
                    setForm({
                      ...
                      recurrenceDays: active
                        ? days.filter((x) => x !== day)
                        : Array.from(new Set([...days, day])).sort(
                            (a, b) => a - b,
                          ),
                    })
                  }
      ...
      {type === "MONTHLY" && (
        <FormRow label="Days of Month (comma-separated)">
          <FormInput
            form={form}
      ...
      {type === "YEARLY" && (
        <div className="grid grid-cols-2 gap-3">
          <FormRow label="Month (1-12)">
            <FormInput
              form={form}
              field="recurrenceMonth"
              setForm={setForm}
              type="number"
            />
          </FormRow>
          <FormRow label="Day (1-31)">
            <FormInput
              form={form}
              field="recurrenceDay"
              setForm={setForm}
