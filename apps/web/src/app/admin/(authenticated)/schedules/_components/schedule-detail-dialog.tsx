...
import { REFUND_POLICIES, WEEKDAYS, GAME_STATUS_COLORS, teamDisplay } from '@/constants';
...
export function ScheduleDetailDialog({ schedule, onClose, onMutated }: ScheduleDetailDialogProps) {
  ...
  const enterEdit = () => {
    ...
    setEditForm({
      ...
      refundPolicy: schedule.refundPolicy, refundPercentage: schedule.refundPercentage || 0,
      url: schedule.url || '', isExclusiveToGold: schedule.isExclusiveToGold,
      teamAName: schedule.teamAName, teamBName: schedule.teamBName,
  ...
  const handleSaveEdit = async () => {
    ...
    let recDays: number[] | null = (f.recurrenceDays as number[])?.length > 0 ? f.recurrenceDays as number[] : null;
    if (f.recurrenceType === 'ONCE') {
      recDays = null;
      if (f.onceDate) {
        const d = new Date(f.onceDate as string);
        recPattern = { year: d.getFullYear(), month: d.getMonth() + 1, day: d.getDate() };
      }
  ...
  return (
    <Dialog open onClose={handleClose} title="Schedule Details" className="max-w-4xl">
      <div className="flex items-center justify-between mb-2">
        ...
        {!editMode && (
          <div className="flex items-center gap-2 shrink-0 ml-4 mb-4">
            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold ${schedule.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}>
              {schedule.isActive ? <><i className="fas fa-circle text-[8px]" /> Active</> : 'Inactive'}
            </span>
      ...
      {detailTab === 'overview' && !editMode && (
        <table className="w-full text-sm">
          <tbody className="divide-y divide-gray-100">
            <tr><td className="py-2 font-medium w-1/3">ID</td><td>{schedule.id}</td></tr>
            <tr><td className="py-2 font-medium">Name</td><td>{schedule.name}</td></tr>
            <tr><td className="py-2 font-medium">Description</td><td>{schedule.description || 'N/A'}</td></tr>
            ...
            <tr><td className="py-2 font-medium">Instant Reserve Cost</td><td>{schedule.instantReservationCost || 0} coins</td></tr>
            <tr><td className="py-2 font-medium">Confirm Window</td><td>{schedule.confirmationWindowMinutes} min</td></tr>
            <tr><td className="py-2 font-medium">Refund Policy</td><td>{schedule.refundPolicy}{schedule.refundPercentage ? ` (${schedule.refundPercentage}%)` : ''}</td></tr>
            <tr><td className="py-2 font-medium">Exclusive to Gold</td><td>{schedule.isExclusiveToGold ? 'Yes' : 'No'}</td></tr>
            <tr><td className="py-2 font-medium">URL</td><td>{schedule.url || 'N/A'}</td></tr>
            <tr><td className="py-2 font-medium">Created</td><td>{formatDate(schedule.createdAt)}</td></tr>
          </tbody>
        </table>
      )}
      {detailTab === 'overview' && editMode && (
        <div className="space-y-3">
          <FormRow label="Name"><FormInput form={editForm} field="name" setForm={setEditForm} /></FormRow>
          <FormRow label="Description"><FormInput form={editForm} field="description" setForm={setEditForm} /></FormRow>
          <div className="grid grid-cols-2 gap-3">
            ...
            <FormRow label="Reserve Cost"><FormInput form={editForm} field="reservationCost" setForm={setEditForm} type="number" /></FormRow>
            <FormRow label="Instant Cost"><FormInput form={editForm} field="instantReservationCost" setForm={setEditForm} type="number" /></FormRow>
            <FormRow label="Slots/Game"><FormInput form={editForm} field="slotsPerGame" setForm={setEditForm} type="number" /></FormRow>
            <FormRow label="Refund Policy"><FormSelect form={editForm} field="refundPolicy" setForm={setEditForm} options={REFUND_POLICIES} /></FormRow>
          </div>
          {editForm.refundPolicy === 'PARTIAL' && <FormRow label="Refund %"><FormInput form={editForm} field="refundPercentage" setForm={setEditForm} type="number" /></FormRow>}
          <FormRow label="URL"><FormInput form={editForm} field="url" setForm={setEditForm} type="url" /></FormRow>
          <FormRow label="Reminder Minutes"><FormInput form={editForm} field="reminderMinutes" setForm={setEditForm} placeholder="30,15" /></FormRow>
          <FormCheckbox form={editForm} field="isExclusiveToGold" setForm={setEditForm} label="Exclusive to Gold" />
          <div className="border-t border-gray-200 pt-3 mt-3">
            <FormCheckbox form={{ propagate }} field="propagate" setForm={(v) => setPropagate(!!v.propagate)} label="Propagate changes now (cancel CREATED games and regenerate)" />
          </div>
      ...
      {detailTab === 'recurrence' && !editMode && (
        <table className="w-full text-sm">
          <tbody className="divide-y divide-gray-100">
            <tr><td className="py-2 font-medium w-1/3">Type</td><td>{schedule.recurrenceType}</td></tr>
            <tr>
              <td className="py-2 font-medium">Days</td>
              <td>
                {schedule.recurrenceType === 'WEEKLY' && schedule.recurrenceDays
                  ? schedule.recurrenceDays.map((d) => WEEKDAYS[d - 1]).join(', ')
                  : schedule.recurrenceType === 'YEARLY' && schedule.recurrencePattern
                    ? `Month ${schedule.recurrencePattern.month}, Day ${schedule.recurrencePattern.day}`
                    : schedule.recurrenceType === 'ONCE' && schedule.recurrencePattern
                      ? (() => {
                          const { year, month, day } = schedule.recurrencePattern;
                          const y = year ?? new Date().getFullYear();
                          return `${y}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
      ...
      {detailTab === 'slots' && !editMode && (
        <div className="overflow-x-auto border border-gray-200 rounded-lg">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {(schedule.slotConfigs || []).sort((a, b) => a.slotNumber - b.slotNumber).map((s) => (
                <tr key={s.slotNumber}>
              ...
              {(schedule.games || []).map((g) => (
                <tr key={g.id}>
                  ...
                  <td className="px-3 py-2">
                    <StatusBadge status={g.status} colorMap={GAME_STATUS_COLORS} />
                  </td>
