...
function defaultCreate(): Record<string, unknown> {
  return {
    ...
    gameCreationTime: '06:00', reservationOpenTime: '',
...
export function CreateScheduleDialog({ open, onClose, onCreated }: CreateScheduleDialogProps) {
  ...
  return (
    <Dialog open={open} onClose={handleClose} title={`Create Schedule - Step ${step} of 3`} className="max-w-2xl">
      <div className="flex items-center gap-2 mb-4">
        {[1, 2, 3].map((s) => (
          <div key={s} className={`flex-1 h-1.5 rounded ${s <= step ? 'bg-indigo-600' : 'bg-gray-200'}`} />
        ))}
      </div>
      ...
      {step === 1 && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <FormRow label="Reservation Cost"><FormInput form={form} field="reservationCost" setForm={setForm} type="number" /></FormRow>
            <FormRow label="Instant Reserve Cost"><FormInput form={form} field="instantReservationCost" setForm={setForm} type="number" /></FormRow>
            <FormRow label="Slots Per Game"><FormInput form={form} field="slotsPerGame" setForm={setForm} type="number" /></FormRow>
            <FormRow label="Refund Policy"><FormSelect form={form} field="refundPolicy" setForm={setForm} options={REFUND_POLICIES} /></FormRow>
          </div>
          {form.refundPolicy === 'PARTIAL' && <FormRow label="Refund %"><FormInput form={form} field="refundPercentage" setForm={setForm} type="number" /></FormRow>}
          <FormRow label="URL"><FormInput form={form} field="url" setForm={setForm} type="url" placeholder="https://youtube.com" /></FormRow>
          <FormRow label="Reminder Minutes"><FormInput form={form} field="reminderMinutes" setForm={setForm} placeholder="30,15" /></FormRow>
          <FormCheckbox form={form} field="isExclusiveToGold" setForm={setForm} label="Exclusive to Gold Subscribers" />
