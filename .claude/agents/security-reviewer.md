...
- Both apps share a single cookie namespace: `access_token` and `refresh_token` (HTTP-only, set by backend), `user` (client-readable, set by frontend)
- JWT strategy reads `access_token` from cookies, then falls back to `Authorization` header
- Role separation is enforced client-side at login (`apps/web/src/lib/api.ts` rejects non-admin); mobile does not currently check role at login
