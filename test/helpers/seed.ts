...
export async function createSchedule(
  ...
  const schedule = scheduleRepository.create({
    ...
    autoStartNextAfterMinutes: overrides.autoStartNextAfterMinutes ?? null,
    gamesPerDay: overrides.gamesPerDay ?? 2,
    spacingAfterFinishMinutes: overrides.spacingAfterFinishMinutes ?? 30,
...
export async function createGameWithSlots(
  ...
  const game = gameRepository.create({
    ...
    url: overrides.url ?? schedule.url ?? null,
